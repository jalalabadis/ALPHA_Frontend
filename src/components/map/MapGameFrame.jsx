...
import MapWinPopup from '../../popup/MapWinPopup';

const MapGameFrame = ({eventData, onGameEnd}) => {
    const [gameResult, setGameResult]=useState(false);
    const gameContainer = useRef(null);

  useEffect(() => {
   //console.log(eventData);
    const rounds = JSON.parse(eventData.rounds);
    const canvasWidth = window.innerWidth * 0.7;
    const army = JSON.parse(eventData?.army);
    const boss = JSON.parse(eventData?.boss);
    let armyHealth = Number(army.healthPoints);
    let bossHealth = Number(boss.healthPoints);
    let gameResult = null;
  ...
  const game = new Phaser.Game(config);
    ...
    function preload() {
      ...
      this.load.image('armyImage', '/assest/img/avatar.jpeg');
      this.load.image('bossImage', '/assest/img/boss-gold.jpg'); 
    ...
    function create() {
            ...
            const playerImg = this.add.image(150, 140, 'armyImage');
     ...
     // Level circle Player
      ...
      const levelText = this.add.text(50, 40, army.level, {
        fontSize: '35px',
        color: '#E0A51D',
...
// playerHealthText
const playerHealthText = this.add.text(70, 248.5, `${armyHealth}/${army.healthPoints}`, { 
  fontSize: '12px',
  color: '#F0DCAE',
  fontFamily: 'MedievalSharp',
  fontWeight: 500,
...
const strengthLabel = this.add.text(30, 290, 'Strength:', { 
  ...
  const strengthValue = this.add.text(240, 290, army.totalStrength, { 
    fontSize: '26px',
    color: '#F0DCAE',
    fontFamily: 'MedievalSharp',
    fontWeight: 500,
...
const vitalityLabel = this.add.text(30, 340, 'Vitality:', { 
  ...
  // Create the text for the value "20" and position it 250px to the right of "Strength:"
  const vitalityValue = this.add.text(240, 340, army.totalVitality, { 
    fontSize: '26px',
    color: '#F0DCAE',
    fontFamily: 'MedievalSharp',
    fontWeight: 500,
...
const luckLabel = this.add.text(30, 390, 'Luck:', { 
  ...
  // Create the text for the value "20" and position it 250px to the right of "Strength:"
  const luckValue = this.add.text(240, 390, army.totalLuck, { 
    fontSize: '26px',
    color: '#F0DCAE',
    fontFamily: 'MedievalSharp',
    fontWeight: 500,
  ...
  const opponentImg = this.add.image(150, 140, 'bossImage');
...
// Level text inside the circle
const levelText2 = this.add.text(50, 40,  boss?.level, {
fontSize: '35px',
color: '#E0A51D',
fontFamily: 'MedievalSharp',
fontWeight: 500,
...
// opponentHealthText
const opponentHealthText = this.add.text(70, 248.5, `${bossHealth}/${boss.healthPoints}`, { 
  fontSize: '12px',
  color: '#F0DCAE',
  fontFamily: 'MedievalSharp',
  fontWeight: 500,
...
// Create the text for the value "20" and position it 250px to the right of "Strength:"
const strengthValue2 = this.add.text(240, 290, boss.totalStrength, { 
fontSize: '26px',
color: '#F0DCAE',
fontFamily: 'MedievalSharp',
fontWeight: 500,
...
// Create the text for the value "20" and position it 250px to the right of "Strength:"
const vitalityValue2 = this.add.text(240, 340, boss.totalVitality, { 
fontSize: '26px',
color: '#F0DCAE',
fontFamily: 'MedievalSharp',
fontWeight: 500,
...
// Create the text for the value "20" and position it 250px to the right of "Strength:"
const luckValue2 = this.add.text(240, 390, boss.totalLuck, { 
fontSize: '26px',
color: '#F0DCAE',
fontFamily: 'MedievalSharp',
fontWeight: 500,
...
rounds.forEach((round, index) => {
  ...
  this?.tweens?.add({
   ...
   if (round.attacker==='player') {
    opponentHealthText.setText(`${round.remainingHealth.Opponent}/${boss.healthPoints}`);
    
    // ProgressBar আপডেট করার আগে ক্লিয়ার করা
    progressBar2.clear();
    progressBar2.fillStyle(0x7E2929, 1);// রং আবার সেট করতে হবে কারণ clear() এটা সরিয়ে ফেলে
    progressBar2.fillRoundedRect(51, 247, 190 * (round.remainingHealth.Opponent / boss.healthPoints), 15, 8);
  
   // যদি playerXP এর পরিমাণ ১০% এর কম হয়, তাহলে progressBar হাইড করে দিন
   if ((round.remainingHealth.Opponent / boss.healthPoints) < 0.1) {
    ...
    playerHealthText.setText(`${round.remainingHealth.Player}/${army.healthPoints}`);
  
    // ProgressBar আপডেট করার আগে ক্লিয়ার করা
    progressBar.clear();
    progressBar.fillStyle(0x7E2929, 1);// রং আবার সেট করতে হবে
    progressBar.fillRoundedRect(51, 247, 190 * (round.remainingHealth.Player / army.healthPoints), 15, 8);
  
    // যদি opponentXP এর পরিমাণ ১০% এর কম হয়, তাহলে progressBar হাইড করে দিন
    if ((round.remainingHealth.Player / army.healthPoints) < 0.1) {
...
>
{gameResult&& <MapWinPopup result={gameResult} event_id={eventData.id} eventData={eventData}   onConfirm={onGameEnd}/>}
</div>
  )
};

export default MapGameFrame;
