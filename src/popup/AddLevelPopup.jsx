...
function AddLevelPopup({id, targetLevel, onClose, onSuccess}) {
    ...
    const handelSubmit=()=>{
      ...
      if(strength&&vitality&&luck&&graphics){
        ...
        axios.post(`${process.env.REACT_APP_SERVER}/equipment/add-level`, formData)
            .then(res => {
              onSuccess(res.data);
...
toast.error('Fill all info');
  ...
  <div style={{display: 'flex', alignItems: 'center', justifyContent: 'flex-start', marginBottom: '10px'}}>
              <h2 style={{margin: 0}}>Add Level {targetLevel}</h2>
     </div>
